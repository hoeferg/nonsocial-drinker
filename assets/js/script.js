...
function createForm() {
  ...
  $howItWorks.setAttribute(
    "class",
    "w3-container w3-vivid-greenish-blue w3-border w3-round-xxlarge w3-card"
  );
  ...
  $description.textContent =
    "Pick your poison from the drinks drop down and indicate whether or not you would like a joke and/or and excuse to go along with it from the options below. You will be shown a list of options on the next page that will aid you in you journey of libation and liberation.";
}
...
function getFavs() {
  ...
  if (localStorage.getItem("favorite-excuses-list") !== null) {
    $favoriteExcuses = JSON.parse(
      localStorage.getItem("favorite-excuses-list")
    );
...
function saveFavoriteDrink() {
  ...
  $refresh.addEventListener("click", function (event) {
    const $drinkDisplay = document.getElementById('genertated-drinks');
...
function saveFavoritejokes() {
  ...
  $refreshJokes.addEventListener("click", function (event) {
    const $jokeDisplay = document.getElementById("generated-jokes");
...
function saveFavoriteExcuses() {
  ...
  $refreshExcuses.addEventListener("click", function (event) {
    const $excuseDisplay = document.getElementById("generated-excuses");
    const $excuseStuff =event.target.getAttribute('data-name');
    refreshDisplay($excuseStuff);
    getExcuse();
  });
